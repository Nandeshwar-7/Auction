...
import { getAuctionBackendHttpUrl, getAuctionRoomWsUrl } from "@/lib/auction-backend";
...
export function useAuctionRoom(roomId: string): UseAuctionRoomResult {
  ...
  const socketUrl = useMemo(() => getAuctionRoomWsUrl(normalizedRoomId), [normalizedRoomId]);
